...
from backend.apps.core.views import home_page
...
def news_checker(request):
    ...
    if((url is not None) and (len(url) > 5)):
        ...
        svc_model = pickle.load(
            open('E:/Highway/prototype-hackathon/backend/apps/api/svc_model.sav', 'rb'))
        mlp_model = pickle.load(open(
            'E:/Highway/prototype-hackathon/backend/apps/api/MLPC_model.sav', 'rb'))
        log_model = pickle.load(
            open('E:/Highway/prototype-hackathon/backend/apps/api/log_model.sav', 'rb'))
        cDict = loadCanonDict()
        ss = SoupStrainer()
        ss.init()
